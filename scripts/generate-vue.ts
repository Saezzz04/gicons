...
const TEMPLATE_PATH = join(ROOT, 'scripts/templates/vue.hbs');
...
for (const brandDir of readdirSync(BRANDS_SRC)) {
  ...
  for (const variant of meta.variants) {
    ...
    const outPath = join(VUE_SRC, `${componentName}.vue`);
...
const indexContent = `// Auto-generated — do not edit manually\n${exports.map((n) => `export { default as ${n} } from './${n}.vue';`).join('\n')}\n`;
writeFileSync(join(VUE_SRC, 'index.ts'), indexContent, 'utf-8');
console.log(`\nGenerated ${exports.length} Vue component(s)`);
